...
def get_local_time(
    ...
    if local_hour >= 24:
        local_hour -= 24
        local_day += 1
    return f"{year}-{month}-{local_day:02d}", local_hour, utc_min
...
def run_update_scores(args: argparse.Namespace) -> None:
    ...
    for mid, score in sorted(scores.items()):
        ...
        new_notes = (
            f"小组{m.group} | 北京时间 {fmt_time(local_h, local_m)} | "
            f"比分: {score} | {m.venue}"
        )
...
def run_add_knockout(args: argparse.Namespace) -> None:
    """STUB: add knockout stage matches to calendar."""
    print("add-knockout: 此功能尚在开发中")
    print("预期功能: 将32强/16强/8强/4强/决赛添加到日历")
    sys.exit(0)
